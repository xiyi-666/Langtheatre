...
func (s *Service) ensureDemoLearningFixtures(userID, _ string) error {
	...
	for _, difficulty := range demoDifficultyBands {
		for _, exam := range []string{"IELTS", "CET4", "CET6"} {
			...
			if _, err := s.store.SaveWritingSession(domain.WritingSession{ID: uuid.NewString(), UserID: userID, Exam: exam, TimeLimitSeconds: 2400, Prompt: prompt, Essay: sample, WordCount: len(strings.Fields(sample)), Status: "COMPLETED", ProgressMessage: "演示评分已准备（非正式成绩）", Evaluation: demoWritingEvaluation(), PromptApproval: contentquality.DemoApproval(), EvaluationApproval: contentquality.DemoApproval(), StartedAt: time.Now().UTC().Add(-30 * time.Minute), SubmittedAt: time.Now().UTC().Add(-5 * time.Minute), CreatedAt: time.Now().UTC(), UpdatedAt: time.Now().UTC()}); err != nil {
				return err
			}
		}
	}
...
func (s *Service) generateDemoWriting(userID, exam string, timeLimitSeconds int, requestedDifficulty string) (domain.WritingSession, error) {
	...
	now := time.Now().UTC()
	session := domain.WritingSession{ID: uuid.NewString(), UserID: userID, Exam: exam, TimeLimitSeconds: timeLimitSeconds, Prompt: demoWritingPrompt(exam, difficulty), PromptApproval: contentquality.DemoApproval(), EvaluationApproval: contentquality.DemoApproval(), Status: "WRITING", ProgressMessage: "演示题目已准备，提交后提供非正式反馈", StartedAt: now, CreatedAt: now, UpdatedAt: now}
...
func (s *Service) submitDemoWriting(userID, sessionID, essay string) (domain.WritingSession, error) {
	...
	session.EvaluationApproval = contentquality.DemoApproval()
